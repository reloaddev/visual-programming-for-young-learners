...
[22] understood the reusability of their functions, but because of limited experience could not see a way to reduce clutter.
...
This conditional if added to the function could have reduced a lot of visual clutter.
...
[22] did instantiation at first when they came to the ``ghost'' and ``bullet'' part of the workshop.
Initially for the bullet did not use instantiation or ``clone''.
And the same did go for the ``ghost'' sprite, which would move towards the player and reset its position when hit.
[22] did end up using instantiation for both the ``ghost'' and ``bullet'' sprites.
Those two sprites were the only intended places to use instantiation.
[22] changed from one ``ghost'' that would change its position upon being hit to a wave spawn system.
With the help of instructors [22] made a ``ghost'' spawner.
The bullets were also changed to use instantiation, to keep up with the ghosts.
...
[22] used basic event handling concepts for the keyboard inputs and collision detection.
The implementation of the event handling used forever loops and if statements, rather than a dedicated event ``broadcast''.
There were attempts at using the ``broadcast'' block, but their implementation failed due to lack of experience.

With help from instructors [22] did implement broadcast events to trigger a new enemy wave after a wave completion.
In [22]'s defense, they were instructed to use for-loops and key-pressed conditionals when making the knight and its movement.
