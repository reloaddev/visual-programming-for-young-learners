...
programming languages Scratch and Godot Orchestrator. Due to the time
constraints we have multiple liability threats that make drawing
decisivie conclusions difficult. Nevertheless, the results we did get
...
programming. The language is optimized for being user friendly and
...
on it. The block-based structure is proven to be intuitive and
children learn to use it much faster than they do traditional
programming languages like Java. However, past studies show that the
children can learn bad habits and get wrong ideas about algorithms,
as they get used to the limited options of Scratch. Writing code in
...
larger programs. Students may be encouraged to start writing code in
...
game designers, audio designers, composors and animators. In fact,
...
or more of these purposes. Thus, we assumed that flow-based editors
would be better for complex structures and would be more suitable for
teaching architechture. For writing code, the most advanced language
we identified was Blueprints for Unreal Engine 5. This flow based
...
clutter. It quickly became clear while using them that large programs
...
block or node of space. But it is still a fact that the boxes just
...
for a weekend. The students were tasked with creating a game in which
the implementation of the software architechture concepts we saught
to teach were essential to finishing the game in time. The workshop
started with a short presentation of the concepts after which the
students were tasked with creating the game. Due to the low amount of
...
Broadly, we judge the structure to have been succesful. After the
...
giving a simple introduction of the concepts. The students were
...
of the students to grasp them fully before the workshop ended. Both
of the students using Godot only had time to implement functions and
variables. While the child using Scratch got way further, none of the
students managed to finish the game in time. We determined this was
...
past. We had assumed this meant that they would have a basic
...
through sequentially. This turned out to not be the case, which meant
that these introductory concepts had to be introduced before teaching
more complex ones. This ended up taking a lot of time, and resulted
...
more of the game than those using Godot Orchestrator. Due to the low
amount of participants, it was not possible to clearly determine if
this was due to advantages built into the language. It seems clear
though that having had prior experience with the language was a huge
advantage. The students using Orchestrator had to be taught how to
...
mechanics immediately. Navigating the user interface of Godot
...
workshop. These are outlined in detail in the results section.
...
games. For example, vectors are commonly used to calculate movement,
and indeed to make the character move using Orchestrator the
programmer is supposed to use vectors. However, the concept of
vectors is, in Denmark, not taught during the public school years. As
such, we had to give the learners using Orchestrator an introductory
explanation of the concept before they could use it. In contrast,
...
making the implementation of movement much easier. Datatypes were
...
implementing complex functions. Scratch has much less functionjality
...
when compare to Godot. Since we had designed the game to be possible
...
without drawbacks. With no way for the developer to set variable
...
concepts. Orchestrator has all the features of Gdscript, the main
...
does not share these problems. In fact, while designing the game that
...
resulted in the workshop being too ambitious. We recommend that
future researchers and educators take this into account when
designing workshops and classes. The assumption should be that the
...
example once a week for a few months. The first lessons would
...
instructions for the computer to execute). Only once the students
...
navigate, and the language being too complex for the students. Here,
...
polished and user friendly then, than Orchestrator is. Unfortunately,
...
learners in the future. Thus  we recommend a structured approach to
...
learners using Godot Orchestrator. Thus, the best language to use for
instructors can not simply be determined by its features. It is
